...
namespace TwitterMood.Common
{
    namespace Localytics
    {
        public class LocalyticsSession
        {
            ...
            private const string libraryVersion = "win_cs_2.0";
            ...
            private static int GetNumberOfStoredSessions()
            {
                ...
                try
                {
                    var folder = store.GetFolderAsync(directoryName).AsTask().Result;
            ...
            {
                var store = GetStore();
                var folder = store.CreateFolderAsync(directoryName, CreationCollisionOption.OpenIfExists).AsTask().Result;
                var file = folder.CreateFileAsync(filename, CreationCollisionOption.OpenIfExists).AsTask().Result;
            ...
            {
                var store = GetStore();
                using (var file = store.OpenStreamForReadAsync(directoryName + @"\" + filename).Result)
                {
            ...
            private static string GetUploadContents()
            {
                ...
                var folder = store.CreateFolderAsync(directoryName, CreationCollisionOption.OpenIfExists).AsTask().GetAwaiter().GetResult();
            ...
            private static void DeleteUploadFiles()
            {
                ...
                foreach (var file in files.Where(x => x.Name.StartsWith(uploadFilePrefix)))
                {
                    file.DeleteAsync().AsTask().GetAwaiter().GetResult();
                }
            }
            ...
            private static string GetSequenceNumber()
            {
                ...
                StorageFile file = FileExists(store, metaFile);
            ...
            private static void SetNextSequenceNumber(string number)
            {
                ...
                else
                {
                    ...
                    using (var fileOut = store.OpenStreamForWriteAsync(metaFile, CreationCollisionOption.ReplaceExisting).Result)
                    {
            ...
            private static StorageFile FileExists(StorageFolder folder, string path)
            {
                try
                {
                    return folder.GetFileAsync(path).AsTask().GetAwaiter().GetResult();
                }
                catch (FileNotFoundException)
                {
                    return null;
            ...
            private static bool IsFileEmpty(StorageFile file)
            {
                using (var stream = file.OpenStreamForReadAsync().Result)
                {
            ...
            {
                var store = GetStore();
                using (var file = store.OpenStreamForReadAsync(directoryName + @"\" + metaFileName).Result)
                {
            ...
            private string GetBlobHeader()
            {
                ...
                blobString.Append("\"dmo\":\"" + "Windows Store Device" + "\",");
            ...
            public void Upload()
            {
                SafeExecute(() =>
                {
                    ...
                    System.Threading.Tasks.Task.Run((Action)BeginUpload);
